...
//     ESP_LOGI(TAG, "RTC Date: %s, Time: %s", date_str, time_str);
...
// criar outra main rodando normal (while 1) e funcionar  e configurar o wifi em modo ap igual acima com http server sempre ligado sem pressionar o botao, mas continuando com senha e escrevendo os dados no sd card normalmente
void app_main() {
    // Inicializa NVS
    esp_err_t ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES ||
    ...
    ESP_LOGI(TAG, "System initialization complete. Data logging started. Wi-Fi AP active.");
}