...
void initialize_rtc(void) {
    ...
    // if (ds1302_read_reg(DS1302_SECONDS_REG) & 0x80) {
        ...
        // --- INÍCIO DO BLOCO DE AJUSTE DE HORA ---

        // 1. Pega a hora em que o código foi compilado
        ...
        // 2. Converte a hora da compilação para segundos
        time_t compile_time_seconds = mktime(&timeinfo);

        // 3. ADICIONA A COMPENSAÇÃO
        compile_time_seconds += 120; 

        // 4. Converte os segundos compensados de volta para o formato de data/hora
        struct tm adjusted_timeinfo;
        localtime_r(&compile_time_seconds, &adjusted_timeinfo);
        
        // 5. Grava a hora AJUSTADA no módulo RTC
        ...
        ds1302_write_reg(0x80, dec_to_bcd(adjusted_timeinfo.tm_sec & 0x7F));
    ...
    if (timeinfo_from_rtc.tm_year > 123 && timeinfo_from_rtc.tm_year < 199) {
        ...
        ESP_LOGI(TAG, "System time synchronized successfully with RTC time.");
    } else {
